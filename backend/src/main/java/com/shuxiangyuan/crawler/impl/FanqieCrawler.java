 ...
 */
@Component
public class FanqieCrawler extends AbstractCrawler {

    private static final String BASE_URL = "https://fanqienovel.com";
    private static final String RANK_BASE_URL = "https://fanqienovel.com/rank";
    ...
    protected String buildRankingUrl(String rankingType) {
        return switch (rankingType) {
            ...
            default -> throw new IllegalArgumentException("番茄小说不支持的排行榜类型: " + rankingType);
        };
    }
    ...
    protected List<NovelInfo> parseRankingDocument(Document doc) {
        ...
        try {
            ...
            log.debug("番茄找到 {} 个排行榜条目", rankItems.size());
    ...
    private NovelInfo extractNovelInfo(Element item, int rank) {
        ...
        String title = extractText(item, ".book-name a, h3 a, h4 a, .title");
        builder.title(title);
        ...
        String sourceUrl = extractUrl(item, "a[href*=/page/]", "href");
